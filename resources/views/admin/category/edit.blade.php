            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="w-full px-6 py-4 bg-white overflow-hidden">
                    <form method="POST" action="{{ route('category.update', $category->id) }}">
                        ...
                        @method('PUT')
                        <div class="py-2">
                            <x-forms.input name="name" :value="$category->name" required="true" label="{{ __('Name') }}" />
                        </div>
                        @unless($category->name == env('APP_SUPER_ADMIN', 'super-admin'))
                            <div class="py-2">
                                <h3
                                ...
                                <x-forms.checkbox id="1" name="tags[]" :items="$tags" :selected="$categoryHasTags" />
                                </div>
                            </div>
                        @endunless
